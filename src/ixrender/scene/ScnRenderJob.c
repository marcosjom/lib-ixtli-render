...
/*
ScnBOOL ScnRenderJob_start(ScnRenderJobRef ref){
    ScnBOOL r = ScnFALSE;
    STScnRenderJobOpq* opq = (STScnRenderJobOpq*)ScnSharedPtr_getOpq(ref.ptr);
    ScnMutex_lock(opq->mutex);
    {
        ...
        //empty objs
        {
            ScnArraySorted_foreach(&opq->cmds.objs, STScnRenderJobObj, o,
                ScnRenderJobObj_destroy(o);
            );
            opq->cmds.objs.use = 0;
        }
        //empty bPropsScns
        if(r && (ScnMemElastic_isNull(opq->cmds.mPropsScns) || !ScnBuffer_clear(opq->cmds.mPropsScns) || !ScnBuffer_invalidateAll(opq->cmds.mPropsScns))){
            printf("ERROR, ScnRenderJob_start::ScnBuffer_clear(bPropsScns) failed.\n");
            r = ScnFALSE;
        }
        //empty bPropsMdls
        if(r && (ScnMemElastic_isNull(opq->cmds.mPropsMdls) || !ScnBuffer_clear(opq->cmds.mPropsMdls) || !ScnBuffer_invalidateAll(opq->cmds.mPropsMdls))){
            printf("ERROR, ScnRenderJob_start::ScnBuffer_clear(bPropsMdls) failed.\n");
            r = ScnFALSE;
        }
        //empty arrays
        if(r){
            //reset state
            ScnRenderCmds_reset(opq);
        }
    }
    ScnMutex_unlock(opq->mutex);
    return r;
...
/*
ScnBOOL ScnRenderJob_end(ScnRenderJobRef ref){
    ScnBOOL r = ScnFALSE;
    STScnRenderJobOpq* opq = (STScnRenderJobOpq*)ScnSharedPtr_getOpq(ref.ptr);
    ScnMutex_lock(opq->mutex);
    {
        ...
        if(r){
            ...
            if(!ScnRenderCmds_addUsedObj(&opq->cmds, ENScnRenderJobObjType_Buff, (ScnObjRef*)&opq->cmds.mPropsScns)){
                printf("ERROR, ScnRenderJob_end::ScnRenderCmds_addUsedObj failed.\n");
                r = ScnFALSE;
            } else if(!ScnRenderCmds_addUsedObj(&opq->cmds, ENScnRenderJobObjType_Buff, (ScnObjRef*)&opq->cmds.mPropsMdls)){
                printf("ERROR, ScnRenderJob_end::ScnRenderCmds_addUsedObj failed.\n");
                r = ScnFALSE;
            }
        }
        ...
        if(r){
            ScnGpuBufferRef bPropsScns = ScnBuffer_getCurrentRenderSlot(opq->cmds.mPropsScns);
            ScnGpuBufferRef bPropsMdls = ScnBuffer_getCurrentRenderSlot(opq->cmds.mPropsMdls);
            if(ScnGpuBuffer_isNull(bPropsScns) || ScnGpuBuffer_isNull(bPropsMdls)){
                //Nothing to render
            } else if(!ScnGpuDevice_render(opq->cmds.gpuDev, bPropsScns, bPropsMdls, opq->cmds.cmds.arr, opq->cmds.cmds.use)){
                printf("ScnRenderJob_end::ScnGpuDevice_render failed.\n");
                r = ScnFALSE;
            }
        }
...
ScnBOOL ScnRenderJob_framebuffPush(ScnRenderJobRef ref, ScnFramebuffRef fbuff){
    ...
    if(!ScnMemElastic_isNull(opq->cmds.mPropsScns)){
        ...
        if(opq->cmds.stackUse < opq->cmds.stack.use){
            ...
            if(f == NULL){
                printf("ERROR, ScnRenderJob_framebuffPush::ScnArray_addPtr failed.\n");
        ...
        if(f != NULL){
            ...
            if(props.ptr.ptr == NULL){
                printf("ERROR, ScnRenderJob_framebuffPush::ScnMemElastic_malloc(STScnGpuFramebuffProps) failed.\n");
            } else if(!ScnRenderFbuffState_addProps(f, &props)){
                printf("ERROR, ScnRenderJob_framebuffPush::ScnRenderFbuffState_addProps failed.\n");
            } else if(!ScnRenderFbuffState_addTransform(f, &t)){
                printf("ERROR, ScnRenderJob_framebuffPush::ScnRenderFbuffState_addTransform failed.\n");
            } else if(!ScnRenderCmds_addUsedObj(&opq->cmds, ENScnRenderJobObjType_Framebuff, (ScnObjRef*)&fbuff)){
                printf("ERROR, ScnRenderJob_framebuffPush::ScnRenderCmds_addUsedObj failed.\n");
            } else {
                //add cmd
                STScnRenderCmd cmd;
                ...
                if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
                    printf("ERROR, ScnRenderJob_framebuffPush::ScnRenderCmds_add failed.\n");
                } else {
                    //push fb props
                    {
...
ScnBOOL ScnRenderJob_framebuffPop(ScnRenderJobRef ref){
    ...
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_framebuffPop::nothing to pop.\n");
    } else {
        r = ScnTRUE;
        opq->cmds.stackUse--;
        ...
        if(opq->cmds.stackUse > 0){
            ...
            if(f->stacks.props.use <= 0){
                ...
                printf("ERROR, ScnRenderJob_framebuffPop::prev-framebuff has not properties in stack.\n");
                r = ScnFALSE;
            } else {
                ...
                cmd.activateFramebuff.viewport  = props->props.viewport;
                if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
                    printf("ERROR, ScnRenderJob_framebuffPop::ScnRenderCmds_add failed.\n");
                } else {
                    r = ScnFALSE;
                }
            }
...
ScnBOOL ScnRenderJob_framebuffPropsPushLockedOpq_(STScnRenderJobOpq* opq, STScnRenderFbuffState* f, const STScnGpuFramebuffProps* pProps){
    ...
    props.ptr   = ScnMemElastic_malloc(opq->cmds.mPropsScns, sizeof(STScnGpuFramebuffProps), NULL);
    if(props.ptr.ptr == NULL){
        printf("ERROR, ScnRenderJob_framebuffPropsPushLockedOpq_::ScnMemElastic_malloc(STScnGpuFramebuffProps) failed.\n");
    } else if(!ScnRenderFbuffState_addProps(f, &props)){
        printf("ERROR, ScnRenderJob_framebuffPropsPushLockedOpq_::ScnRenderFbuffState_addProps failed.\n");
    } else {
        //add cmd
        STScnRenderCmd cmd;
        ...
        if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
            printf("ERROR, ScnRenderJob_framebuffPropsPushLockedOpq_::ScnRenderCmds_add failed.\n");
        } else {
            //push fb props
            STScnGpuFramebuffProps* buffPtr = (STScnGpuFramebuffProps*)props.ptr.ptr;
            *buffPtr = props.props;
...
ScnBOOL ScnRenderJob_framebuffPropsPush(ScnRenderJobRef ref, const STScnGpuFramebuffProps pProps){
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_framebuffPropsPush::no active framebuffer.\n");
    } else {
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        //add cmd
        if(f->stacks.props.use <= 0){
            //program logic error (should be previously validated)
            printf("ERROR, ScnRenderJob_framebuffPropsPush::framebuff has not properties in stack.\n");
        } else {
...
ScnBOOL ScnRenderJob_framebuffPropsPop(ScnRenderJobRef ref){
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_framebuffPropsPop::no active framebuffer.\n");
    } else {
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        //add cmd
        if(f->stacks.props.use <= 1){
            printf("ERROR, ScnRenderJob_framebuffPropsPop::framebuff has not properties to pop.\n");
        } else {
            //add cmd
            ...
            if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
                printf("ERROR, ScnRenderJob_framebuffPropsPop::ScnRenderCmds_add failed.\n");
            } else {
                r = ScnTRUE;
            }
        }
...
ScnBOOL ScnRenderJob_setTextureLockedOpq_(STScnRenderJobOpq* opq, STScnRenderFbuffState* f, const ENScnGpuTextureIdx idx, ScnTextureRef tex){
    ...
    if(ScnTexture_isSame(f->active.texs.refs[idx], tex)){
        ...
        r = ScnTRUE;
    } else if(!ScnRenderCmds_addUsedObj(&opq->cmds, ENScnRenderJobObjType_Texture, (ScnObjRef*)&tex)){
        printf("ERROR, ScnRenderJob_setTexture::ScnRenderCmds_addUsedObj failed.\n");
    } else {
        //add cmd
        STScnRenderCmd cmd;
        ...
        if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
            printf("ERROR, ScnRenderJob_setTexture::ScnRenderCmds_add failed.\n");
        } else {
...
ScnBOOL ScnRenderJob_setTexture(ScnRenderJobRef ref, const ENScnGpuTextureIdx idx, ScnTextureRef tex){
    ...
    if(opq->cmds.stackUse <= 0){ //cmds-queues are per-framebuffer
        printf("ERROR, ScnRenderJob_setTexture::no active framebuffer.\n");
    } else if(idx < 0 || idx >= ENScnGpuTextureIdx_Count){
        printf("ERROR, ScnRenderJob_setTexture::idx-out-of-bounds.\n");
    } else {
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
...
SC_INLN ScnBOOL ScnRenderJob__addSetVertexBuffIfNecesaryLockedOpq_(STScnRenderJobOpq* opq, STScnRenderFbuffState* f, ScnVertexbuffRef vbuff){
    ...
    //new command
    {
        ...
        if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
            printf("ERROR, ScnRenderJob__addSetVertexBuffIfNecesaryLockedOpq_::ScnRenderCmds_add failed.\n");
            return ScnFALSE;
        } else if(!ScnRenderCmds_addUsedObj(&opq->cmds, ENScnRenderJobObjType_Vertexbuff, (ScnObjRef*)&vbuff)){
            printf("ERROR, ScnRenderJob__addSetVertexBuffIfNecesaryLockedOpq_::ScnRenderCmds_addUsedObj failed.\n");
            return ScnFALSE;
        } else {
...
#define ScnRenderJob_addCommandDrawVertsLockeOpq_(OPQ, FRAMEBUFF_STATE, DRAW_CMD, VERTEX_TYPE, VERTEX_TYPE_IDX) \
    if(ScnVertexbuffs_isNull((DRAW_CMD)->vbuffs)){ \
        printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithPropsLockedOpq_::cmd without ScnVertexbuffs.\n"); \
        r = ScnFALSE; \
    } else { \
        if(!ScnRenderJob__addSetVertexBuffIfNecesaryLockedOpq_(OPQ, FRAMEBUFF_STATE, ScnVertexbuffs_getVertexBuff((DRAW_CMD)->vbuffs, VERTEX_TYPE))){ \
            printf("ERROR, ScnRenderJob__addSetVertexBuffIfNecesaryLockedOpq_ failed.\n"); \
            r = ScnFALSE; \
        } else { \
            STScnRenderCmd cmd; \
...
STScnAbsPtr ScnRenderJob_model2dAdd_addGpuModelPropsLockedOpq_(STScnRenderJobOpq* opq, STScnRenderFbuffState* f, const STScnGpuModelProps2d* const transform){
    ...
    if(r.ptr == NULL){
        printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithTransformLockedOpq_::ScnMemElastic_malloc failed.\n");
    } else {
        STScnRenderCmd cmd;
        cmd.cmdId = ENScnRenderCmd_SetTransformOffset;
        cmd.setTransformOffset.offset = r.idx;
        if(!ScnRenderCmds_add(&opq->cmds, &cmd)){
            printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithTransformLockedOpq_::ScnRenderCmds_add failed.\n");
            r.ptr = NULL;
        } else {
...
ScnBOOL ScnRenderJob_model2dAdd_addCommandsWithTransformLockedOpq_(STScnRenderJobOpq* opq, STScnRenderFbuffState* f, const STScnGpuModelProps2d* const transform, const STScnModel2dCmd* const cmds, const ScnUI32 cmdsSz){
    ...
    if(tNPtr.ptr == NULL){
        printf("ERROR, ScnRenderJob_model2dAdd_addGpuModelPropsLockedOpq_::ScnMemElastic_malloc failed.\n");
    } else {
...
ScnBOOL ScnRenderJob_model2dAdd_addCommandsWithPropsLockedOpq_(void* data, const STScnGpuModelProps2d* const props, const STScnModel2dCmd* const cmds, const ScnUI32 cmdsSz){
    ...
    if(opq != NULL && props != NULL){
        if(opq->cmds.stackUse <= 0){
            printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithPropsLockedOpq_::no active framebuffer.\n");
        } else {
            STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
            if(f->stacks.transforms.use < 1){
                printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithPropsLockedOpq_::missing parent transform.\n");
            } else {
                SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
                SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
                if(!ScnRenderJob_model2dAdd_addCommandsWithTransformLockedOpq_(opq, f, props, cmds, cmdsSz)){
                    printf("ERROR, ScnRenderJob_model2dAdd_addCommandsWithPropsLockedOpq_::ScnRenderJob_model2dAdd_addCommandsWithTransformLockedOpq_ failed.\n");
                } else {
                    r = ScnTRUE;
                }
            }
...
ScnBOOL ScnRenderJob_node2dPush(ScnRenderJobRef ref, ScnNode2dRef node){
    return ScnRenderJob_node2dPropsPush(ref, ScnNode2d_getProps(node));
}
...
ScnBOOL ScnRenderJob_node2dPropsPush(ScnRenderJobRef ref, const STScnNode2dProps nodeProps){
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_node2dPropsPush::no active framebuffer.\n");
    } else {
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        if(f->stacks.transforms.use < 1){
            printf("ERROR, ScnRenderJob_node2dPropsPush::missing parent transform.\n");
        } else {
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
            ...
            if(!ScnRenderFbuffState_addTransform(f, &tN)){
                printf("ERROR, ScnRenderJob_node2dPropsPush::ScnRenderFbuffState_addTransform failed.\n");
            } else {
...
ScnBOOL ScnRenderJob_node2dPop(ScnRenderJobRef ref){
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_node2dPop::no framebuffer is active.\n");
    } else {
        SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
        SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        if(f->stacks.transforms.use < 2){
            printf("ERROR, ScnRenderJob_node2dPop::nothing to pop.\n");
        } else {
...
ScnBOOL ScnRenderJob_model2dAddWithNode(ScnRenderJobRef ref, ScnModel2dRef model, ScnNode2dRef node){ //equivalent to jobNode2dPush() + jobModelAdd() + jobNodePropsPop()
    return !ScnModel2d_isNull(model) && !ScnNode2d_isNull(node) ? ScnRenderJob_model2dAddWithNodeProps(ref, model, ScnNode2d_getProps(node)) : ScnFALSE;
}
...
ScnBOOL ScnRenderJob_model2dAddWithNodeProps(ScnRenderJobRef ref, ScnModel2dRef model, const STScnNode2dProps nodeProps){ //equivalent to jobNodePropsPush() + jobModelAdd() + jobNodePropsPop()
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_model2dAddWithNodeProps::no active framebuffer.\n");
    } else {
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        if(f->stacks.transforms.use < 1){
            printf("ERROR, ScnRenderJob_model2dAddWithNodeProps::missing parent transform.\n");
        } else {
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
            const STScnGpuModelProps2d t = ScnNode2dProps_toGpuTransform(&nodeProps);
            ...
            if(!ScnModel2d_sendRenderCmds(model, &tN, &itf, opq)){
                printf("ERROR, ScnRenderJob_model2dAddWithNodeProps::ScnModel2d_sendRenderCmds failed.\n");
            } else {
                r = ScnTRUE;
            }
        }
...
ScnBOOL ScnRenderJob_model2dAdd(ScnRenderJobRef ref, ScnModel2dRef model){
    ...
    ScnMutex_lock(opq->mutex);
    if(opq->cmds.stackUse <= 0){
        printf("ERROR, ScnRenderJob_model2dAdd::no framebuffer is active.\n");
    } else {
        SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
        SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
        STScnRenderFbuffState* f = &opq->cmds.stack.arr[opq->cmds.stackUse - 1];
        if(f->stacks.transforms.use < 1){
            printf("ERROR, ScnRenderJob_model2dAdd::render props unavailable.\n");
        } else {
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsScns)) //program logic error
            SCN_ASSERT(!ScnMemElastic_isNull(opq->cmds.mPropsMdls)) //program logic error
            const STScnGpuModelProps2d tPrnt = f->stacks.transforms.arr[f->stacks.transforms.use - 1];
            ...
            if(!ScnModel2d_sendRenderCmds(model, &tPrnt, &itf, opq)){
                printf("ERROR, ScnRenderJob_framebuffPop::ScnModel2d_sendRenderCmds failed.\n");
            } else {
                r = ScnTRUE;
            }
        }
...
void ScnRenderJob_cmdMaskModePush(ScnRenderJobRef ref){
    ...
    ScnMutex_lock(opq->mutex);
    {
        ...
        ScnArray_addValue(opq->ctx, &opq->cmds.cmds, cmd, STScnRenderCmd);
    }
    ScnMutex_unlock(opq->mutex);
}
